---
title: "GitHub - pypatterns/python-cqrs: Event-Driven Architecture Framework"
source: "https://github.com/vadikko2/python-cqrs"
author:
published:
created: 2026-05-07
description: "Event-Driven Architecture Framework. Contribute to pypatterns/python-cqrs development by creating an account on GitHub."
tags:
  - clippings
  - "vault-scout"
  - "source:tier2-catalog"
  - "kind:article"
  - "score:7"
---

WarningBreaking Changes in v5.0.0
...
This package is a fork of the diator
project (documentation) with several enhancements, ordered by importance:
Core framework

Redesigned the event and request mapping mechanism to handlers;
EventMediator for handling Notification and ECST events coming from the bus;
bootstrap for easy setup;
Transaction Outbox, ensuring that Notification and ECST events are sent to the broker;
Orchestrated Saga pattern for distributed transactions with automatic compensation and recovery;
StreamingRequestMediator and StreamingRequestHandler for streaming requests with real-time progress updates;
Chain of Responsibility with CORRequestHandler for processing requests through multiple handlers in sequence;
Parallel event processing with configurable concurrency limits.

Also

Typing: Pydantic v2.* and IRequest/IResponse interfaces — use Pydantic-based, dataclass-based, or custom Request/Response implementations.
Broker: Kafka via aiokafka.
...
Optional dependencies (see pyproject.toml for full list):
...
For full setup with DI, events, and outbox, see the documentation and the examples directory.
Request and Response Types
...
A complete example can be found in request_response_types.py
Request Handlers
...
TipBy default, the command handler does not return any result, but it is not mandatory.
...
the read model.
TipThe read model can be constructed based on domain events produced by the Command Handler.
...
Complete example: CoR Mermaid Diagrams
Mapping
...
Simple example
Practical example with FastAPI
...
# Generate Class diagram showing type structure
class_diagram = generator.class_diagram()
Complete example: Saga Mermaid Diagrams
Producing Notification Events
...
WarningIt is important to note that producing events using the events property parameter does not guarantee message delivery
to the broker.
...
recommend using the Transaction Outbox pattern,
...
Transactional Outbox
The package implements the Transactional Outbox
...
A complete example can be found in
the documentation
TipYou can specify the name of the Outbox table using the environment variable OUTBOX_SQLA_TABLE.
...
TipHowever, it can be implemented
...
TipI recommend reading the useful
...
Run tests: pytest.
Install pre-commit and run hooks: pre-commit install && pre-commit run --all-files.

The project uses ruff for linting and pyright for type checking.
Changelog
Release notes and migration guides are published on GitHub Releases.
License
